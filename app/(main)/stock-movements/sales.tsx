...
import { buildSalesItemsTableReportPrintHtml } from "../../../components/reports/sales/SalesItemsReportPrintTemplate";
...
export default function SalesItemsScreen() {
  ...
  const buildCurrentSalesItemsReportMeta = () => {
    ...
    items.push({ label: "Transactions", value: summary.totalTransactions });
  ...
  const handlePrintSalesItemsTable = async () => {
    try {
      ...
      Alert.alert("Print failed", error instanceof Error ? error.message : "Failed to print sales items report.");
    }
  };
