...
export default function StockAdjustmentScreen() {
  ...
  const buildCurrentStockAdjustmentReportMeta = () => {
    ...
    try {
      ...
      const payload = {
        ...
        meta: isTable ? buildCurrentStockAdjustmentReportMeta() : [
          { label: "Code", value: selectedRow?.adjustment_code },
          { label: "Product", value: selectedRow?.product_name },
      ...
      await logClientActivity({
        ...
        tableName: "tbl_stock_movements",
        description: "Sent stock adjustment report via email.",
      });
  ...
  const handlePrintStockAdjustmentTable = async () => {
    try {
      const html = buildStockAdjustmentTableReportPrintHtml({
        rows: Array.isArray(filteredRows) ? filteredRows : [],
  ...
  const handlePrintStockAdjustmentDetail = async () => {
    if (!selectedRow) return;

    try {
      const html = buildStockAdjustmentDetailReportPrintHtml({
        adjustment: selectedRow,
        generatedAt: new Date(),
        generatedBy: roleName,
      });
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      >
            <View style={styles.detailModalHeader}>
              ...
              <ExportDropdownMenu
                variant="detail"
                onExportPdf={handlePrintStockAdjustmentDetail}
      ...
      <SendEmailModal
        ...
        reportTitle={emailTarget === "table" ? "Stock Adjustment List" : "Adjustment Detail"}
...
const styles = StyleSheet.create({
  ...
  detailModalHeader: { borderBottomWidth: 1, borderBottomColor: "#eef2f7", padding: 14, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10 },
  detailModalTitle: { padding: 0, borderBottomWidth: 0, flex: 1 },
