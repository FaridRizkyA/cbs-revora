import { Alert, useCallback, useEffect, useMemo, useState } from "react";
...
export default function ProductsScreen() {
  ...
  const isNarrowDetail = width < 980;
  ...
  const submitProductForm = async () => {
    ...
    const code = productForm.product_code.trim().toUpperCase();
    ...
    if (!actorId) {
      Alert.alert("Session", "Session user not found. Please re-login.");
      return;
    }
    if (!supplierId) {
    ...
    if (!Number.isFinite(price) || price < 0) {
      Alert.alert("Validation", "Selling price must be greater than or equal to 0.");
      return;
    }
    if (!Number.isInteger(minStock) || minStock < 0) {
      Alert.alert("Validation", "Minimum stock must be an integer greater than or equal to 0.");
      return;
    }
  ...
  const toggleProductStatus = (product: Product, nextState: "Y" | "N") => {
    const title = nextState === "N" ? "Deactivate Product" : "Activate Product";
    const message = nextState === "N" ? "Deactivate this product?" : "Activate this product?";
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            await fetchProducts();
            Alert.alert("Success", nextState === "N" ? "Product deactivated successfully." : "Product activated successfully.");
          } catch (error) {
            Alert.alert("Error", error instanceof Error ? error.message : "Failed to update product status.");
          }
        },
      },
    ]);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
                        ...
                        <View style={styles.actionMenu}>
                          ...
                          {canManageInventoryMaster(roleName) ? (
                            <>
                              ...
                              <Pressable style={[styles.actionOutlineBtn, styles.actionOutlineDanger]} onPress={() => { setOpenActionProductId(null); toggleProductStatus(item, "N"); }}>
      ...
      <Modal visible={Boolean(selectedProduct)} transparent animationType="fade">
        <View style={styles.modalBackdrop}>
          <View style={styles.detailModalCard}>
            <Text style={styles.modalTitle}>Product Detail</Text>
            <View style={[styles.metaGrid, isNarrowDetail && styles.metaGridMobile]}>
              <View style={[styles.productImageWrap, isNarrowDetail ? styles.productImageWrapMobile : styles.productImageWrapDesktop]}>
                {selectedProduct?.product_image ? (
                  <Image source={{ uri: selectedProduct.product_image }} style={styles.productImage} />
                ) : (
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.formModalCard}>
            ...
            <ScrollView contentContainerStyle={styles.formBody}>
              ...
              <View style={styles.formGrid}>
                <View style={styles.formFieldHalf}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(value) => setProductForm((prev) => ({ ...prev, product_code: value.toUpperCase() }))}
                    editable={!isEdit}
                    placeholder="PRD-001"
                    placeholderTextColor="#94a3b8"
              ...
              <View style={styles.formField}>
                ...
                <TextInput
                  ...
                  onChangeText={(value) => setProductForm((prev) => ({ ...prev, product_image: value }))}
            ...
            <View style={styles.formActionRow}>
              ...
              <Pressable style={styles.formSaveBtn} onPress={submitProductForm} disabled={saving}>
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            {inactiveProducts.length === 0 ? (
              <Text style={styles.modalMeta}>No inactive product data.</Text>
            ) : (
              <View style={styles.modalList}>
                {inactiveProducts.map((item) => (
                  <View key={item.id_product} style={styles.inactiveRow}>
                    <Text style={styles.modalMeta}>
                      {item.product_code} - {item.product_name}
                    </Text>
                    {canManageInventoryMaster(roleName) ? (
                      <Pressable style={styles.activateBtn} onPress={() => toggleProductStatus(item, "Y")}>
...
const styles = StyleSheet.create({
  ...
  priceCol: { width: "16%" },
  stockCol: { width: "10%" },
  actionCol: { width: "8%", textAlign: "center" },
  ...
  metaGridMobile: { flexDirection: "column", alignItems: "center" },
  metaStack: { flex: 1, gap: DETAIL_ROW_GAP, width: "100%", height: DETAIL_GRID_HEIGHT },
  productImageWrap: {
    borderRadius: 10,
    overflow: "hidden",
    backgroundColor: "#f1f5f9",
  ...
  productImageWrapDesktop: { width: 250, height: DETAIL_GRID_HEIGHT },
  productImageWrapMobile: { width: 250, height: DETAIL_GRID_HEIGHT, alignSelf: "center" },
